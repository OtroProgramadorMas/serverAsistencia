...
export const AprendicesByIdFicha = async (ctx: any) => {
  ...
  try {
    ...
    // Usando la función mejorada del admin branch
    const result = await obtenerAprendicesPorFicha(id);
...
export const getAprendicesActivos = async (ctx: any) => {
  const { response } = ctx;
  try {
    const result = await listarAprendizActivos();

    if (!result.success) {
      response.status = 404;
      response.body = { success: false, msg: result.msg || "No se encontraron aprendices activos" };
      return;
    }
    ...
    console.error("Error en getAprendicesActivos:", error);
    response.status = 500;
    response.body = { success: false, msg: "Error interno del servidor" };
  }
};
...
export const getEstadosAprendiz = async (ctx: any) => {
  ...
  try {
    const estados_aprendiz = await listarEstadosAprendiz();
    response.status = 200;
